...
for R in xrange(0, 856):
    p_R = 2000 * np.sqrt(
        float(this_roj_df['c1'] / (R - this_roj_df['intercept'])))
    c_R = 1650

    P = 855 - R
    p_P = 2000 * np.sqrt(
        float(this_poj_df['c1'] / (P - this_poj_df['intercept'])))
    c_P = 2000
...
for i, p in enumerate(p_vec):
    ...
    SW_ROJ_demand = SW_ROJ_values['c1'] / (p ** 2) + SW_ROJ_values['intercept']
    NW_ROJ_values = df[(df['region'] == 'NW') & (df['product'] == 'ROJ')]
    NW_ROJ_demand = NW_ROJ_values['c1'] / (p ** 2) + NW_ROJ_values['intercept']
    P05_ROJ_demand[i] = float(SW_ROJ_demand) + float(NW_ROJ_demand)

    SW_POJ_values = df[(df['region'] == 'SW') & (df['product'] == 'POJ')]
    SW_POJ_demand = SW_POJ_values['c1'] / (p ** 2) + SW_POJ_values['intercept']
    NW_POJ_values = df[(df['region'] == 'NW') & (df['product'] == 'POJ')]
    NW_POJ_demand = NW_POJ_values['c1'] / (p ** 2) + NW_POJ_values['intercept']
...
for i, p in enumerate(p_vec):
    ...
    SE_ROJ_demand = SE_ROJ_values['c1'] / (p ** 2) + SE_ROJ_values['intercept']
    MA_ROJ_values = df[(df['region'] == 'MA') & (df['product'] == 'ROJ')]
    MA_ROJ_demand = MA_ROJ_values['c1'] / (p ** 2) + MA_ROJ_values['intercept']
    NE_ROJ_values = df[(df['region'] == 'NE') & (df['product'] == 'ROJ')]
    NE_ROJ_demand = NE_ROJ_values['c1'] / (p ** 2) + NE_ROJ_values['intercept']
    P03_ROJ_demand[i] = float(SE_ROJ_demand) + float(MA_ROJ_demand) + float(NE_ROJ_demand)

    SE_POJ_values = df[(df['region'] == 'SE') & (df['product'] == 'POJ')]
    SE_POJ_demand = SE_POJ_values['c1'] / (p ** 2) + SE_POJ_values['intercept']
    MA_POJ_values = df[(df['region'] == 'MA') & (df['product'] == 'POJ')]
    MA_POJ_demand = MA_POJ_values['c1'] / (p ** 2) + MA_POJ_values['intercept']
    NE_POJ_values = df[(df['region'] == 'NE') & (df['product'] == 'POJ')]
    NE_POJ_demand = NE_POJ_values['c1'] / (p ** 2) + NE_POJ_values['intercept']
