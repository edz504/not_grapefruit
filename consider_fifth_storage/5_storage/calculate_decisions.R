...
print('S21 storage capacity: ')
sum(by.region[by.region$region == 'SE', 2])

print('S51 storage capacity: ')
sum(by.region[by.region$region %in% c('NE', 'MA'), 2])
...
fla.p09 <- sum(df[(df$region %in% c('MW')) &
           ...
           sum(fla$predicted_demand)
fla.p09
fla.s21 <- sum(df[(df$region == 'SE') &
                   df$product %in% c('ORA'),]$predicted_demand) /
           sum(fla$predicted_demand)
fla.s21
fla.s51 <- sum(df[(df$region %in% c('NE', 'MA')) &
                   df$product %in% c('ORA'),]$predicted_demand) /
           sum(fla$predicted_demand)
fla.s51
...
s21.fcoj.futures.ship <- sum(df[(df$product == 'FCOJ' &
                                 df$region == 'SE'), ]$predicted_demand) /
                         sum(df[df$product == 'FCOJ', ]$predicted_demand)
s21.fcoj.futures.ship
...
s51.fcoj.futures.ship <- sum(df[(df$product == 'FCOJ' &
                                 df$region %in% c('NE', 'MA')),]$predicted_demand) /
                         sum(df[df$product == 'FCOJ', ]$predicted_demand)
...
# P03 -> S21, S51 split
p03.s21.poj <- sum(df[(df$product == 'POJ' &
                       df$region == 'SE'), ]$predicted_demand) /
               sum(df[(df$region %in% c('NE', 'MA', 'SE')) &
                       df$product == 'POJ', ]$predicted_demand)
p03.s21.poj

p03.s21.fcoj <- sum(df[(df$product == 'ROJ' &
                       df$region == 'SE'), ]$predicted_demand) /
                sum(df[(df$region %in% c('NE', 'MA', 'SE')) &
                        df$product == 'ROJ', ]$predicted_demand)
p03.s21.fcoj

p03.s51.poj <- sum(df[(df$product == 'POJ' &
                       df$region %in% c('NE', 'MA')), ]$predicted_demand) /
               sum(df[(df$region %in% c('NE', 'MA', 'SE')) &
                       df$product == 'POJ', ]$predicted_demand)
p03.s51.poj

p03.s51.fcoj <- sum(df[(df$product == 'ROJ' &
                        df$region %in% c('NE', 'MA')), ]$predicted_demand) /
                sum(df[(df$region %in% c('NE', 'MA', 'SE')) &
                        df$product == 'ROJ', ]$predicted_demand)
p03.s51.fcoj
...
# Reconstitution
s21.recon <- sum(df[(df$product == 'ROJ' &
                     df$region == 'SE'), ]$predicted_demand) /
